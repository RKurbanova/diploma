...
export default function CatalogPage({user}) {
    ...
    const updateData = useCallback(() => {
        ...
        if (sort) {
            ...
            if (sort.includes('risk')) {
                ...
                // [...new Set(deal.Investments.map(({UserID}) => UserID))].length
                newFilteredDeals.sort(({Investments: Investments1}, {Investments: Investments2}) => {
                    const currentBalance1 = [...new Set(Investments1.map(({UserID}) => UserID))].length
                    const currentBalance2 = [...new Set(Investments2.map(({UserID}) => UserID))].length
                    return numberComparator(currentBalance1, currentBalance2)
                })
            }
        }
    ...
    return <Space style={{padding: '20px', width: '100%'}} direction='vertical'>
        <Space style={{justifyContent: 'space-between', width: '100%'}}>
            ...
            />
            {
                user?.IsPromoted ? <Space>
                    <Link to='/deal/create'><Button type='primary'>Создать проект или сделку</Button></Link>
                    <Link to={`/user/${user.ID}/deals`}><Button type='primary'>Мои проекты или сделки</Button></Link>
                </Space>: null
            }
        </Space>
        <Tag style={{marginBottom: '10px'}} color='default'>У каждого проекта или сделки есть рейтинг рискованности, определенный экспертами - <Rate disabled value={5} /></Tag>
