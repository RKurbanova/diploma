...
export default function DealsStagesApprovePage({user}) {
    const {data, isLoading: isDealsLoading} = useGetAllDealsQuery()

    const deals = useMemo(() => {
        return data?.filter(({Stages}) => {
            return Stages.some(({IsSubmited}) => IsSubmited)
        })
    }, [data])
    ...
    return <Space style={{padding: '20px', width: '100%'}} direction='vertical'>
        <Space style={{justifyContent: 'space-between', width: '100%'}}>
            <PageHeader
                ...
                title={<span className="profile-header">Заявки на подтверждение этапа</span>}
            />
        </Space>
